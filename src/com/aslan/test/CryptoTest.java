...
public class CryptoTest {
    ...
    */
    ...
    public CryptoTest(int keySize) { // keySize:byte
        N = keySize;
     ...
     * @param ky 原始密匙Y
     */
    public void init(byte[] kx, byte[] ky) throws Exception {
    ...
    private void init(byte[] kx, byte[] ky, int index) throws Exception {
        int l = 0;
        for (int i = 1; i <= round; i++) {
            for (int r = 0; r < N; r++) {
                ky[r] = (byte) (FKy.call(kx[l], ky[r], null) & 255);
                kx[l] = (byte) (FKx.call(kx[l], ky[r], null) & 255);
                l = (++l) % N;
            }
            l = P >> i;
        }
    ...
    class XKey implements Function {
        ...
        @Override
        public int call(int x, int y, OutputStream out, int... values) throws Exception {
            return A * x * x + B * y + C;
        }
    }
     ...
     * @param Y 明文Y
     */
    public void encrypt(byte[] X, byte[] Y, OutputStream out) throws Exception {
        int i = 0;
        for (int r = 1, k = 0; r <= round; r++, k++) {
            for (int j = 0; j < N; j++) {
                Y[j] = (byte) FKy.call(X[i], Y[j], out);
                X[i] = (byte) FKx.call(X[i], Y[j], out);
                i = (++i) % N;
            }
            i = P >> r;
        }
    ...
    Function Ex = new XEnc();
    Function Ey = new YEnc();

    public class XEnc implements Function {
        @Override
        public int call(int x, int y, OutputStream out, int... values) throws Exception {
            int z = values[0];
            int v = S[(x ^ y) & 0xFF] ^ z;
    ...
    public class YEnc implements Function {
        @Override
        public int call(int x, int y, OutputStream out, int... values) throws Exception {
            int z = values[0];
            int v = S[(x ^ y) & 0xFF] ^ z;
    ...
    // ---------------------------------------------------------------------------
    interface Function {
        int call(int x, int y, OutputStream out, int... values) throws Exception;
    }

    public static void microChangeTest() throws Exception {
        int length = 1024;
        byte[] KX = new byte[length];
        byte[] KY = new byte[length];
        byte[] MX = new byte[length];
        byte[] MY = new byte[length];
        CryptoTest demo = new CryptoTest(length);

        demo.init(KX, KY); // 初始化密匙
        ...
        for (int i = 0; i < 1; i++) {
            byte[] TX = MX.clone();
            byte[] TY = MY.clone();
            demo.encrypt(TX, TY, null);
            Utils.print(TX, "\t");
            Utils.print(TY, "\n");
        }

        // demo.show = true;
        // FileOutputStream oa = new FileOutputStream("F:\\temp\\a.txt.txt");
        // byte[] TX = MX.clone();
        // byte[] TY = MY.clone();
        // TX[0] = (byte) 2;
        // demo.encrypt(TX, TY, oa);
        // FileOutputStream ob = new FileOutputStream("F:\\temp\\b.txt");
        // TX = MX.clone();
        // TY = MY.clone();
        // TX[0] = (byte) 0xa5;
        // demo.encrypt(TX, TY, ob);
    }
